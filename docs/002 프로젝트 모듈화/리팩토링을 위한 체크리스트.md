...
- [x] 시간대 쿼리(`utils/timeParams.ts`)와 `toApiDateTimeWithZone` 사용법을 문서화하고 중복 계산을 제거. (`docs/시간대_파라미터_가이드.md`)

### 4. 상태/컨텍스트 정리
- [x] Schedule/Task 캐시 컨텍스트를 `src/shared/contexts`로 이동 완료(이벤트 헬퍼 타입 안전화는 추후 진행 필요).
- [x] ScheduleViewState와 TimePreferences 의존 관계를 문서화하고 hook vs context 중 하나로 일원화. (`docs/시간대_뷰상태_정리.md`)
- [x] MiniPlayerBar, TodayPage, SchedulesTabPage에서 공유되는 일정-작업 동기화 로직을 공통 훅(`useScheduleTaskSync`)으로만 사용하도록 정리.
...
- [x] 포인터/터치 드래그 로직이 긴 `SchedulesTabPage`를 표현 컴포넌트와 로직 훅으로 분리해 가독성을 개선. (`useScheduleDrag` 도입)

### 6. 날짜·시간·알림
- [x] 모든 날짜 계산이 `TimePreferencesContext`와 `utils/datetime`을 경유하는지 확인하고, `toISOString` 직접 호출이 있으면 `toApiDateTimeWithZone` 등으로 교체. (전역 검색 결과 실사용 없음, `httpClient` 디버그 로그만 `toISOString` 사용)
- [x] FCM 설정(`src/firebase/config.ts`, `messaging.ts`)을 `.env` 기반으로 치환하고, 프로덕션 빌드에서만 초기화되도록 가드 추가.
- [x] 서비스워커 등록(main.tsx)과 푸시 토큰 발급 흐름을 문서화하고, mock 모드에서 비활성화되는지 확인. (`docs/푸시_서비스워커_플로우.md`)
